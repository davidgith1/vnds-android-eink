...
/**
 * The per-row "⋮" popup on a library entry: "Get info from VNDB" and "Visit VNDB page" -- same
 * flat, non-animated popup style as {@link LaunchChooserDialog}.
 */
public final class VnRowMenuDialog {
    ...
    private VnRowMenuDialog() {
    }

    /** @param hasGuide whether a completion guide is already imported for this VN -- "View guide"
     *                  is only shown when there's one to view. */
    public static void show(Context context, String title, boolean hasGuide, Listener listener) {
        ...
        addRow(context, content, R.string.edit_title, () -> {
            dialog.dismiss();
        ...
        addDivider(context, content);
        addRow(context, content, R.string.export_saves, () -> {
            dialog.dismiss();
            listener.onExportSaves();
        });
        addDivider(context, content);
        addRow(context, content, R.string.import_saves, () -> {
            dialog.dismiss();
            listener.onImportSaves();
        });
        addDivider(context, content);
